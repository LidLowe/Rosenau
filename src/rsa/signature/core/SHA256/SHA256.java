package rsa.signature.core.SHA256;
...
public class SHA256
{
    ...
    private static int[] compress(int[] padded)
    {
        ...
        for (int i = 0; i < padded.length; i += 16)
        {
            ...
            for (int t = 16; t < 64; t++)
            {
                w[t] = BitwiseOperations.smallSigma1(w[t - 2]) + w[t - 7] + BitwiseOperations.smallSigma0(w[t - 15]) + w[t - 16];
            }
            ...
            for (int t = 0; t < 64; t++)
            {
                int t1 = hVal + BitwiseOperations.bigSigma1(e) + BitwiseOperations.ch(e, f, g) + Constants.K[t] + w[t];
                int t2 = BitwiseOperations.bigSigma0(a) + BitwiseOperations.maj(a, b, c);
